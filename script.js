...
function createCanvas(ix) {
    ...
    html2canvas(parent).then(canvas => {
        ...
        for (let i = 0; i < elemt.length; i++) {
            ...
            // elemt[i].style.transition = "all ease .5s"
        }
...
function animation(datas, index) {
    ...
    function ejecut() {
        setTimeout(() => {
            ...
            if (!debug) {
                countAudio += 1
                let audio = new Audio(`https://www.google.com/logos/fnbx/thanos/thanos_dust_${index+1}.mp3`)
                audio.play()
                frames.forEach((frame, i) => {
...
function init() {
    ...
    gauntlet.onclick = () => {
        ...
        if (!click) {
            ...
            let audio = new Audio("https://www.google.com/logos/fnbx/thanos/thanos_reverse_sound.mp3")
            canvas.style.position = "absolute"
            canvas.style.cursor = "pointer"
            gauntlet.appendChild(canvas)
            audio.play()
            animationImage(canvas, "https://www.google.com/logos/fnbx/thanos/thanos_time.png", res => {
        ...
        click = false
        let canvas = document.createElement("canvas")
        let audio = new Audio("https://www.google.com/logos/fnbx/thanos/thanos_snap_sound.mp3")
        canvas.style.position = "absolute"
        canvas.style.cursor = "pointer"
        gauntlet.appendChild(canvas)
        audio.play()
        animationImage(canvas, "https://www.google.com/logos/fnbx/thanos/thanos_snap.png", res => {
